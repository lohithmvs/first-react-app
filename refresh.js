...
const pro=arr.map(val=>{ console.log(val);
    products.map(val1=>{
    if(val1.category==val){
        console.log(val1.title);
    }
})
});
console.log("2 problem");
const rate1=products.filter(val=>{
    return val.rating.rate>4.0;
});
console.log(rate1);
console.log("3 problem");
const inst=products.map(val=>{
    val.isCart=false; 
});
...
const sorting1=products.sort((a,b)=>{
